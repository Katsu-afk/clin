...
import { Context } from '..';
import { getBasket } from '../http/clothingAPI';
...
const Basket = observer(() => {
    ...
    useEffect(() => {
        getBasket().then(data => clothing.setBaskets(data))
    }, [])

    // ----- Считаем общую сумму, которую юзер набрал в корзину ------- //

    let prices = 0;
    {clothing.basket.map(price =>
        prices += Number(price.clothing.price)
    )}
    return (
    <Container
    ...
    >
        ...
        {clothing.basket.map(product =>
            <Card className="d-flex w-100 p-2 justify-content-center mb-2" key={product.id}>
                <Row className="d-flex w-100">
                    <Col>
                        <div className="d-flex flex-row align-items-center">
                            <img src={process.env.REACT_APP_API_URL + product.clothing.img} width={50}  />
                            <h1 className="pl-3">{product.clothing.name}</h1>
                        </div>
                    </Col>
                    <Col>
                        <div className="d-flex h-100 flex-row justify-content-end align-items-center">
                            <h2 className="font-weight-light">{product.clothing.price} рублей</h2>
                        </div>
                    </Col>
                </Row>
